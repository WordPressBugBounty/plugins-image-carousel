...
function icp_clear_cache_ajax() {
	...
	if( ! check_ajax_referer( 'icp_clear_cache', 'security' ) )
		...
		$files = glob( EWIC_FULL_BFITHUMB_UPLOAD_DIR.'*.{jpg,png,bmp,jpeg}', GLOB_BRACE ); // get all file names
		...
		if ( count( glob( EWIC_FULL_BFITHUMB_UPLOAD_DIR.'*.{jpg,png,bmp,jpeg}', GLOB_BRACE ), COUNT_NORMAL ) === 0 ) {
...
body.rtl #icp_free_plugins_container .plugin-card .desc > p {
	margin-left: 0;
}
	...
	<div id="the-list">
		...
		<div id="<?php echo $plugin["slug"]; ?>" class="plugin-card drop-shadow lifted">
            ...
            <?php if ( isset( $plugin["slug"] ) && $plugin["slug"] == 'easy-media-gallery' ) {echo '<div class="most_popular"></div>';} ?>
				<a href="<?php echo esc_url( $details_link ); ?>" class="icp-free-plugin-icon thickbox plugin-icon"><img width="128" height="128" src="<?php echo esc_attr( $plugin_icon_url ) ?>" /></a>
