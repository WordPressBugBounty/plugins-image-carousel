...
<script type="text/javascript">
	...
	<form id="icp_lightbox_form" method="post" action="#">
		...
		<tr valign="top">
        <th scope="row"><?php _e('Caption SlideUp/SlideDown Effect', 'image-carousel');?></th>
        <td><select name="icp_carousel_caption_effect" id="icp_carousel_caption_effect">
            <option value="true" <?php selected( esc_attr( icp_get_option('icp_carousel_caption_effect') ), 'true' ); ?>>Yes</option>
            <option value="false" <?php selected( esc_attr( icp_get_option('icp_carousel_caption_effect') ), 'false' ); ?>>No</option>
            </select>
        <p class="description"><?php  _e('Use this option to set the Carousel captions effect', 'image-carousel'); ?></p>
        </td>
        </tr>
		...
		<tr valign="top">
        <th scope="row"><?php _e('Arrow Navigation Always Visible', 'image-carousel');?></th>
        <td><select name="icp_carousel_nav_fixed" id="icp_carousel_nav_fixed">
            <option value="true" <?php selected( esc_attr( icp_get_option('icp_carousel_nav_fixed') ), 'true' ); ?>>Yes</option>
            <option value="false" <?php selected( esc_attr( icp_get_option('icp_carousel_nav_fixed') ), 'false' ); ?>>No</option>
            </select>
        <p class="description"><?php  _e('Use this option to set Carousel navigation always visible or only visible on hover', 'image-carousel'); ?></p>
        </td>
        </tr>

		<tr valign="top">
        <th scope="row"><?php _e('Arrow Navigation Hide on Mobile Devices', 'image-carousel');?></th>
        <td><select name="icp_carousel_hide_nav_mobile" id="icp_carousel_hide_nav_mobile">
            <option value="true" <?php selected( esc_attr( icp_get_option('icp_carousel_hide_nav_mobile') ), 'true' ); ?>>Yes</option>
            <option value="false" <?php selected( esc_attr( icp_get_option('icp_carousel_hide_nav_mobile') ), 'false' ); ?>>No</option>
            </select>
        <p class="description"><?php  _e('Use this option to hide Carousel navigation on mobile devices', 'image-carousel'); ?></p>
        </td>
        </tr>

		<tr valign="top">
        <th scope="row"><?php _e('Arrow Navigation Color', 'image-carousel');?></th>
        <td><select name="icp_carousel_nav_col" id="icp_carousel_nav_col">
            <option value="black" <?php selected( esc_attr( icp_get_option('icp_carousel_nav_col') ), 'black' ); ?>>Black</option>
            <option value="white" <?php selected( esc_attr( icp_get_option('icp_carousel_nav_col') ), 'white' ); ?>>White</option>
            </select>
        <p class="description"><?php  _e('Use this option to set Carousel navigation color. Default: Black', 'image-carousel'); ?></p>
        </td>
        </tr>
        
		<tr valign="top">
        <th scope="row"><?php _e('Slideshow Transitions', 'image-carousel');?></th>
        <td><select style="width:155px;" name="icp_carousel_slide_effect" id="icp_carousel_slide_effect">
							<?php 
							
$easing = array( "none", "easeInQuad", "easeOutQuad", "easeInOutQuad", "easeInCubic", "easeOutCubic", "easeInOutCubic", "easeInQuart", "easeOutQuart", "easeInOutQuart", "easeInQuint", "easeOutQuint", "easeInOutQuint", "easeInSine", "easeOutSine", "easeInOutSine", "easeInExpo", "easeOutExpo", "easeInOutExpo", "easeInCirc", "easeOutCirc", "easeInOutCirc", "easeInElastic" , "easeOutElastic", "easeInOutElastic", "easeInBack", "easeOutBack", "easeInOutBack", "easeInBounce", "easeOutBounce", "easeInOutBounce");
		...
		<tr valign="top" >
        <th><?php _e('Slideshow Transitions Speed', 'image-carousel');?></th>
        
        <?php $t_speed = ( icp_get_option('icp_carousel_transition_speed') ? icp_get_option('icp_carousel_transition_speed') : '2500' ); ?>
        
        <td><input style="width:55px" name="icp_carousel_transition_speed" type="text" id="icp_carousel_transition_speed" value="<?php echo esc_attr_e( $t_speed ); ?>" size="40" /><span> px</span>
        <p class="description"><?php  _e('Set the Carousel transition duration (in ms). Default: 2500', 'image-carousel'); ?></p>
        </td>
        </tr>
      ...
      <?php
		...
		if ( is_dir( $req_plugin ) ) {
			...
			if ( !is_plugin_active( 'gallery-lightbox-slider/gallery-lightbox-lite.php' ) ) {
				...
				$gallery_action = '<p class="icpnotesfailed"><strong>Gallery Lightbox Lite</strong> is not active. You can activate from <a href="'.$activate.'">here</a></p>';
				
				$gallery_option = ' style="display:none;" ';
				
			}
			
			$plugin_data = get_plugin_data( $req_plugin . '/gallery-lightbox-lite.php' );
			
			if ( ! icp_glg_supported_version() ) {
				
				$update_url = wp_nonce_url(
					add_query_arg(
						array(
							'action' => 'upgrade-plugin',
							'plugin' => 'gallery-lightbox-slider/gallery-lightbox-lite.php'
							),
					admin_url( 'update.php' )
						),
					'upgrade-plugin_gallery-lightbox-slider/gallery-lightbox-lite.php'
					);
				
				$gallery_action = '<p class="icpnotesfailed">The version of <strong>Gallery Lightbox Lite</strong> ('.$plugin_data['Version'].') is not compatible with Image Carousel. Please update the plugin first to be able to use the Lightbox effect. You can <a href="'.$update_url.'">click here</a> to update.</p>';
				$gallery_option = ' style="display:none;" ';
			...
			$gallery_action = '<p class="icpnotesfailed"><strong>Gallery Lightbox Lite</strong> plugin is not installed. Please install first from <a href="'.$install_url.'">here</a></p>';	
			
		}
		?>
        <td>
        	<?php echo $gallery_action; ?>
        	<select <?php echo $gallery_option;?>name="icp_carousel_use_lightbox" id="icp_carousel_use_lightbox">
        ...
        <p <?php echo $gallery_option;?>class="description"><?php  _e('Use this option to enable / disable Lighbox mode', 'image-carousel'); ?></p>
	...
	<table class="tbl_custom">
        ...
        <td><input data-formname="icp_lightbox_form" data-nonce="<?php echo wp_create_nonce( "icp_form_settings" ); ?>" type="submit" value="<?php _e('Save Changes'); ?>" class="button button-primary icp_form_submit" /><span id="loader_icp_lightbox_form"></span><span style="display:none;position:relative;color:#148919; margin-left:11px; top:6px;" class="set_icp_lightbox_form icp_save_status">Settings Saved</span></td>
                    ...
                    <table class="tbl_custom">
                    	...
                    	<tr valign="top">
                        	<td>
                    		<h2><a href="https://ghozylab.com/plugins/ordernow.php?order=addons-image-carousel" target="_blank"><span class="icl-btn icl-btn-danger"><span style="margin-right: 5px;top:5px;margin-top: 3px;" class="dashicons dashicons-cart"></span>UPGRADE NOW !</span></a></h2>
                    	  </td>
                    	</tr>
